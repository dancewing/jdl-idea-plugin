...
public class JdlFoldingBuilder extends FoldingBuilderEx implements DumbAware {

  @NotNull
  @Override
  public FoldingDescriptor[] buildFoldRegions(@NotNull PsiElement root, @NotNull Document document, boolean quick) {
    // Initialize the group of folding regions that will expand/collapse together.
    FoldingGroup group = FoldingGroup.newGroup(JdlAnnotator.SIMPLE_PREFIX_STR);
    ...
    for (final PsiLiteralExpression literalExpression : literalExpressions) {
      ...
      if (value != null && value.startsWith(JdlAnnotator.SIMPLE_PREFIX_STR + JdlAnnotator.SIMPLE_SEPARATOR_STR)) {
        ...
        final List<JdlProperty> properties = JdlUtil.findProperties(project, key);
  ...
  public String getPlaceholderText(@NotNull ASTNode node) {
    ...
    if (node.getPsi() instanceof PsiLiteralExpression) {
      ...
      final List<JdlProperty> properties = JdlUtil.findProperties(nodeElement.getProject(), key);
